...
public class ContactAdapter extends RecyclerView.Adapter<ContactAdapter.NameViewHolder> {
    ...
    public void onBindViewHolder(@NonNull NameViewHolder holder, int position) {
        ...
        TextView tvTipo =  view.findViewById(R.id.tvTipo); // Nuevo TextView tvTipo
        Button btnVer = view.findViewById(R.id.btnVer);

        // Load the image using Picasso
        String imageUrl = "https://assets.pokemon.com/assets/cms2/img/pokedex/full/" + item.getNumberContact() + ".png";
        Picasso.get().load(imageUrl).into(ivContactImage);

        tvNombre.setText(item.getNameContact());
        tvNumero.setText(item.getNumberContact());
        tvTipo.setText(item.getTipo()); // Establecer el valor del atributo tipo
        ...
        btnVer.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                intent.putExtra("imagen", imageUrl); // Pasar la URL de la imagen
                intent.putExtra("tipo", clickedItem.getTipo()); // Pasar el atributo tipo
